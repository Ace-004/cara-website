...
function getCurrentSession() {
    try { return JSON.parse(localStorage.getItem('cara_session')); } catch { return null; }
}
...
const Cart = {
    ...
    add(item) {
        ...
        const idx = items.findIndex(p => p.id === item.id && p.size === item.size);
        if (idx >= 0) {
...
(function () {
    ...
    addBtn.addEventListener('click', () => {
        ...
        const id = (name + '|' + price + '|' + (img || '')) + (size ? ('|' + size) : '');
...
(function () {
    ...
    function render() {
        ...
        items.forEach(p => {
            ...
            tr.innerHTML = `
                <td><button class="remove-item" data-id="${p.id}" data-size="${p.size || ''}"><i class=\"fa-regular fa-circle-xmark\"></i></button></td>
                <td><img src="${p.img}" alt="" style="width:70px"></td>
                <td>${p.name}${p.size ? ` - ${p.size}` : ''}</td>
                <td>${fmt(p.price)}</td>
                <td><input class="cart-qty" type="number" min="1" value="${p.qty}" data-id="${p.id}" data-size="${p.size || ''}"></td>
...
(function () {
    function isLoggedIn() { const s = getCurrentSession(); return !!(s && s.isLoggedIn); }
    ...
    if (isLoggedIn()) {
        ...
        loginLi.innerHTML = '<button id="logout-btn" style="background-color: #ef4444; color: white; padding: 10px 18px; border: none; border-radius: 8px; cursor: pointer;">Logout</button>';
        ...
        if (!loginLi.querySelector('a')) {
            loginLi.innerHTML = '<a href="auth.html"><button style="background-color: #088178; color: white; padding: 10px 18px; border: none; border-radius: 8px; cursor: pointer;">Login</button></a>';
        }
    }
})();
...
(function () {
    ...
    function openModal() { modal.style.display = 'flex'; updateExtra(); }
    function closeModal() { modal.style.display = 'none'; successBox.style.display = 'none'; successBox.textContent = ''; }
    ...
    function updateExtra() {
        ...
        if (selected === 'upi') {
            ...
            paymentExtra.innerHTML = '<input name="upiId" required style="width:100%;padding:10px;border:1px solid #e1e1e1;border-radius:8px;" type="text" placeholder="Enter UPI ID (e.g. name@upi)">';
        } else if (selected === 'card') {
            paymentExtra.style.display = 'block';
            paymentExtra.innerHTML = '<input name="cardNumber" required style="width:100%;padding:10px;border:1px solid #e1e1e1;border-radius:8px;margin-bottom:8px;" type="text" inputmode="numeric" placeholder="Card Number (16 digits)"><div style="display:flex; gap:8px;"><input name="cardExpiry" required style="flex:1;padding:10px;border:1px solid #e1e1e1;border-radius:8px;" type="text" placeholder="MM/YY"><input name="cardCvv" required style="flex:1;padding:10px;border:1px solid #e1e1e1;border-radius:8px;" type="text" inputmode="numeric" placeholder="CVV"></div>';
        } else {
    ...
    placeBtn.addEventListener('click', () => {
        ...
        if (method === 'upi') {
            const upi = (paymentExtra.querySelector('input[name="upiId"]')?.value || '').trim();
            const upiOk = /^[\w.\-]{2,}@[\w\-]{2,}$/.test(upi);
            if (!upiOk) { alert('Please enter a valid UPI ID (e.g. name@upi)'); return; }
        } else if (method === 'card') {
            const num = (paymentExtra.querySelector('input[name="cardNumber"]')?.value || '').replace(/\s+/g, '');
            const exp = (paymentExtra.querySelector('input[name="cardExpiry"]')?.value || '').trim();
            const cvv = (paymentExtra.querySelector('input[name="cardCvv"]')?.value || '').trim();
            const numOk = /^\d{16}$/.test(num);
            const expOk = /^(0[1-9]|1[0-2])\/(\d{2})$/.test(exp);
            const cvvOk = /^\d{3,4}$/.test(cvv);
            if (!numOk) { alert('Enter a valid 16-digit card number'); return; }
            if (!expOk) { alert('Enter a valid expiry in MM/YY'); return; }
            if (!cvvOk) { alert('Enter a valid 3 or 4 digit CVV'); return; }
        }
        ...
        successBox.textContent = `Order placed successfully! Amount charged: $${totals.total.toFixed(2)}. Thank you for shopping.`;
...
(function () {
    ...
    document.addEventListener('click', (e) => {
        ...
        setTimeout(() => { btn.classList.remove('added'); btn.setAttribute('title', oldTitle); }, 800);
    });
})();
...
(function () {
    ...
    function render() {
        ...
        let orders = [];
        try { orders = JSON.parse(localStorage.getItem('cara_orders') || '[]'); } catch { }
        if (email) {
                            ...
                            <tr>
                                <td style=\"padding:6px 0;\">
                                    <div style=\"display:flex; align-items:center; gap:8px;\">
                                        <img src=\"${p.img}\" alt=\"\" style=\"width:50px; height:50px; object-fit:cover; border-radius:6px;\"/>
                                        <div>${p.name}${p.size ? ` - ${p.size}` : ''}</div>
                                    </div>
                                </td>
                                <td style=\"padding:6px 0;\">${p.qty}</td>
                                <td style=\"padding:6px 0;\">${fmt(p.price)}</td>
                                <td style=\"padding:6px 0;\">${fmt(p.price * p.qty)}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
                <div style="text-align:right; margin-top:8px;"><strong>Total: ${fmt(o.total)}</strong></div>
            `;
...
(function () {
    ...
    document.addEventListener('click', (e) => {
        ...
        const url = `sproduct.html?name=${encodeURIComponent(name)}&price=${encodeURIComponent(price)}&img=${encodeURIComponent(img)}`;
...
(function () {
    ...
    if (priceParam && !isNaN(parseFloat(priceParam))) priceEl.textContent = `$${parseFloat(priceParam).toFixed(2)}`;
